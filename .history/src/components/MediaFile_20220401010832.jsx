...
export default function MediaFile({ className, array, grid }) {
  return (
    <div className={`border-b-[1px] pb-8 py-5 ${className} space-y-3`}>
      ...
      <div className={`grid grid-cols-${grid} pt-3 relative`}>
        {array.map((i) => (
          <img key={i} className="w-20 h-20 rounded-sm" src={Image} alt="" />
        ))}

        <div className="w-20 h-20 rounded-sm bg-[#0000006b] absolute bottom-0 right-0 text-white flex justify-center font-semibold p-7 ">
          2+
        </div>
      </div>
    </div>
